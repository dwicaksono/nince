...
const app = new Hono()
  ...
  .post(
    ...
    async (c) => {
      ...
      const transactionsBulkDelete = db.$with('transactions_bulk_delete').as(
        db
      ...
      const data = await db
        .with(transactionsBulkDelete)
        .delete(transactions)
        .where(
          inArray(
            transactions.id,
            sql`(select id from ${transactionsBulkDelete})`,
          ),
        )
  ...
  .patch(
    ...
    async (c) => {
      ...
      const transactionstoUpdate = db.$with('transactions_update').as(
        db
          .select({ id: transactions.id })
          .from(transactions)
          .innerJoin(accounts, eq(transactions.accountId, accounts.id))
  ...
  .delete(
    ...
    async (c) => {
      ...
      const transactionstoDelete = db.$with('transactions_delete').as(
        db
          .select({ id: transactions.id })
          .from(transactions)
          .innerJoin(accounts, eq(transactions.accountId, accounts.id))
